...
router.post('/post-create', isLoggedIn, fileUploader.single('post-file-upload'), (req, res, next) => {
    ...
    if (!title || !category || !description) {
        res.render('posts/create.hbs', { errorMessage: 'Category, title and descritpion must be filled in.' });
        return;
    }
...
router.post('/post/:postId/edit', isLoggedIn, fileUploader.single('post-file-upload'), (req, res, next) => {
    const { postId } = req.params;
    ...
    Post.findByIdAndUpdate(postId, { title, category, description, mediaUrl }, { new: true })
        .then(updatedPost =>
